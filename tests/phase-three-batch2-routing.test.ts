...
vi.mock('@/lib/auth/auth-middleware', () => ({
  requireAuth: async () => ({
    user: {
      id: 'phase-three-batch2-provider',
      role: 'provider',
      email: 'phase-three-batch2@veranote.local',
    },
    isAuthenticated: true,
    providerIdentityId: 'phase-three-batch2-provider',
    tokenSource: 'header',
  }),
}));
...
describe('phase 3 batch 2 clinical routing', () => {
  it('supports all 9 batch 2 families with the expected answer mode and pressure-turn persistence', async () => {
