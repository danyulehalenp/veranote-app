...
vi.mock('@/lib/auth/auth-middleware', () => ({
  requireAuth: async () => ({
    user: {
      id: 'phase-three-batch3-provider',
      role: 'provider',
      email: 'phase-three-batch3@veranote.local',
    },
    isAuthenticated: true,
    providerIdentityId: 'phase-three-batch3-provider',
    tokenSource: 'header',
  }),
}));
...
const genericFallbackMarkers = [
  ...
  'persistent psychotic disorder featuring characteristic positive and/or negative symptoms',
];
...
describe('phase 3 batch 3 clinical routing', () => {
  it('supports all 9 batch 3 full-document workflows with chart-ready persistence', async () => {
    const cases = selectVeraProviderQuestionCases(undefined, [...phaseThreeBatchThreeCategories]);
    ...
    for (const caseDefinition of cases) {
      ...
      const scriptedTurns = [
        ...
        arr.findIndex((item) => item.label === turn.label && item.prompt === turn.prompt) === index
      ));
      ...
      for (const turn of scriptedTurns) {
        ...
        for (const marker of genericFallbackMarkers) {
          expect(includesPhrase(text, marker), `${caseDefinition.id} ${turn.label} generic/meta marker: ${marker}`).toBe(false);
        }
