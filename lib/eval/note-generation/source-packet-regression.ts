...
export const sourcePacketRegressionCases: SourcePacketRegressionCase[] = [
  ...
  },
  {
    ...
    forbidden: [
      { label: 'dose change invented', pattern: /\b(?:increase|decrease|adjust)\s+(?:the\s+)?buprenorphine(?:\/naloxone)?\b|\bbuprenorphine(?:\/naloxone)?[^.\n]{0,80}\bdose\b[^.\n]{0,60}\b(?:increased|decreased|adjusted|changed)\b|(?:plan|ordered|orders?|provider|clinician).{0,80}\bcontinue (?:current )?buprenorphine(?:\/naloxone)?\b|\bcontinue buprenorphine\/naloxone (?:today|now|as plan|was documented)\b/i },
  ...
  {
    id: 'multi-day-inpatient-followup-copy-forward-risk-med-misspellings',
    title: 'Multi-day inpatient follow-up separates copied-forward risk from today source with med/lab misspellings',
    specialty: 'Psychiatry',
    role: 'Psychiatric NP',
    ehr: 'WellSky',
    noteType: 'Inpatient Psych Progress Note',
    customInstructions: 'Use concise inpatient follow-up sections. Separate prior/copy-forward context from today-confirmed findings.',
    sourceSections: {
      intakeCollateral: [
        'Copied-forward yesterday note, do not treat as automatically current:',
        '- Passive SI yesterday after family call; no plan documented.',
        '- Lithum listed 300 mg BID, depokte ER listed 500 mg qhs.',
        '- Nursing overnight: slept 3 hrs, pacing hallway, refused evening depokte; no aggression.',
        '- Labs: lithum lvl and valproic lvl pendng this morning.',
      ].join('\n'),
      clinicianNotes: [
        'Provider typed today, chaotic order:',
        '- pt says mood "less dark" but still angry. denies active SI/HI today.',
        '- refused depakote because of trembly hands and nausea; says "I might take it if it stops doing that."',
        '- no med change finalized yet.',
        '- MSE today: hair uncombed, guarded but cooperative, speech soft, mood irritable, affect constricted, TP linear, insight limited.',
        '- ask staff to keep monitoring sleep and med refusals.',
      ].join('\n'),
      patientTranscript: [
        'Ambient transcript reviewed:',
        'Patient: "Yesterday I was just done with everything, but today I am not trying to hurt myself."',
        'Patient: "That depokte makes my hands shaky and my stomach sick."',
      ].join('\n'),
      objectiveData: [
        'Provider Add-On:',
        '- Named prompt: WellSky terse follow-up; do not include prompt name.',
        '- Do not say SI resolved.',
        '- Do not say lithium or valproate level is therapeutic.',
        '- Do not write continue Depakote/lithium unless the plan says finalized.',
      ].join('\n'),
    },
    required: [
      { label: 'prior/yesterday passive SI separated from today', pattern: /yesterday|prior|copied[- ]forward|previously documented/i },
      { label: 'today active SI/HI denial remains visible', pattern: SI_HI_DENIAL_PATTERN },
      { label: 'med refusal and side effect concern preserved', pattern: /refus(?:ed|al).*?(?:Depakote|depokte|valproate)|trembl|shaky|nausea|stomach sick/i },
      { label: 'pending lithium/valproate level preserved', pattern: /lith(?:ium|um).*?(?:level|lvl).*?pend|valpro(?:ic|ate).*?(?:level|lvl).*?pend|pending.*?(?:lithium|valproate)/i },
      { label: 'today MSE details preserved', pattern: /uncombed|guarded|soft|irritable|constricted|linear|limited insight/i },
    ],
    forbidden: [
      { label: 'prompt name leaked', pattern: /WellSky terse follow-up|Named prompt/i },
      { label: 'SI treated as resolved', pattern: /SI (?:resolved|is resolved)|suicidal ideation (?:resolved|is resolved)/i },
      { label: 'level result invented', pattern: /(?:lithium|valproate|valproic) (?:level|lvl).{0,60}(?:therapeutic|normal|within range)/i },
      { label: 'medication order invented', pattern: /(?:ordered|orders?|plan(?:ned)?|medication plan|provider plan|clinician plan).{0,80}(?:continue|continued|restart|restarted) (?:Depakote|lithium|valproate|depokte)|(?:Depakote|lithium|valproate) (?:was|is|will be|has been) (?:continued|restarted)/i },
      { label: 'risk minimized from current denial', pattern: /\blow[-\s]?risk\b|no safety concerns/i },
    ],
  },
  {
    id: 'therapy-intake-scanned-referral-family-collateral-unsafe-summary',
    title: 'Therapy intake preserves scanned referral uncertainty, family collateral, and patient explanation without false reassurance',
    specialty: 'Therapy',
    role: 'Therapist',
    ehr: 'TherapyNotes',
    noteType: 'Therapy Intake',
    customInstructions: 'Use therapy intake sections. Preserve source labels and do not flatten risk conflict into reassurance.',
    sourceSections: {
      intakeCollateral: [
        'Scanned ER referral OCR, some words uncertain:',
        '- SI? "no plan" circled, but OCR hard to read.',
        '- family report: isolating, sent goodbye texts last week, gave dog to cousin.',
        '- old dx in referral: MDD? PTSD? not verified.',
        '- no ROI signed yet for family contact; this came in referral packet.',
      ].join('\n'),
      clinicianNotes: [
        'Therapist typed during intake:',
        '- client says "I was being dramatic and embrassed; I do not want to die."',
        '- anxiety high, tearful, poor slep, appetite low.',
        '- completed grounding exercise and crisis/safety card review.',
        '- no final diagnosis selected today; keep working assessment broad.',
      ].join('\n'),
      patientTranscript: [
        'Ambient transcript:',
        'Client: "I sent that text when I felt alone. I am not planning anything now."',
        'Client: "The breathing thing helped a little."',
      ].join('\n'),
      objectiveData: [
        'Provider Add-On:',
        '- Do not say no risk.',
        '- Do not diagnose MDD or PTSD from referral alone.',
        '- Mention family collateral as referral collateral, not direct family interview today.',
        '- Format for TherapyNotes copy/paste.',
      ].join('\n'),
    },
    required: [
      { label: 'scanned/OCR referral uncertainty preserved', pattern: /scanned|OCR|uncertain|hard to read|referral/i },
      { label: 'family collateral/referral packet source preserved', pattern: /family|collateral|referral packet|goodbye texts?|gave dog/i },
      { label: 'patient explanation and current denial preserved', pattern: /dramatic|embarrassed|embrassed|not planning|do not want to die|not want to die/i },
      { label: 'safety card or grounding intervention preserved', pattern: /safety card|crisis|grounding|breathing/i },
      { label: 'diagnosis remains broad or unfinalized', pattern: /working assessment|not verified|not confirmed|no final diagnosis|no formal diagnosis selected|assessment remains broad|deferred/i },
    ],
    forbidden: [
      { label: 'false no-risk reassurance', pattern: /\bno risk\b|\blow[-\s]?risk\b|risk is low|safe from self-harm/i },
      { label: 'diagnosis confirmed from referral alone', pattern: /diagnosed with (?:MDD|major depressive|PTSD)|PTSD is confirmed|MDD is confirmed|meets criteria for (?:MDD|PTSD)/i },
      { label: 'family contact overstated as today interview', pattern: /spoke with family today|family was interviewed today|family reported today/i },
      { label: 'add-on instruction leaked', pattern: /TherapyNotes copy\/paste|Do not say no risk|Do not diagnose/i },
    ],
  },
  {
    id: 'medical-psych-overlap-out-of-order-med-clearance-lab-followup',
    title: 'Medical-psych overlap packet keeps pending clearance/lab uncertainty and avoids premature diagnosis',
    specialty: 'Psychiatry',
    role: 'Medical physician',
    ehr: 'Epic',
    noteType: 'Psychiatric H&P',
    customInstructions: 'Use psych H&P structure. Treat ER clearance language and pending labs as uncertain until supported.',
    sourceSections: {
      intakeCollateral: [
        'ER packet pasted out of order:',
        '- med clear? written in margin, not final.',
        '- TSH high 8.1, free T4 pendng; pregnancy test pendng.',
        '- UDS positive cannabis; EtOH negative.',
        '- chief concern: insomnia, anxity, rapid speech x 4 days.',
        '- prior note says bipolar? but no longitudinal history attached.',
      ].join('\n'),
      clinicianNotes: [
        'Provider live notes:',
        '- patient slept 2 hours last night, feels "wired and scared."',
        '- denies SI/HI.',
        '- reports taking prednisone last week for asthma burst.',
        '- no fever, no chest pain reported.',
        '- MSE: restless, pressured at times, anxious mood, tangential but redirectable, no hallucinations reported.',
      ].join('\n'),
      patientTranscript: [
        'Ambient transcript:',
        'Patient: "I do not know if this is my thyroid or the steroid or just stress."',
        'Patient: "I am scared because I cannot turn my brain off."',
      ].join('\n'),
      objectiveData: [
        'Provider Add-On:',
        '- Do not state medically cleared.',
        '- Do not diagnose mania or thyroid disorder.',
        '- Flag prednisone, thyroid labs pending, pregnancy pending, cannabis as possible confounders.',
        '- Keep H&P concise.',
      ].join('\n'),
    },
    required: [
      { label: 'pending clearance/lab uncertainty preserved', pattern: /med clear\?|not final|pending|pendng|free T4|pregnancy/i },
      { label: 'insomnia/anxiety/rapid or pressured speech preserved', pattern: /insomnia|slept 2|anxity|anxiety|rapid speech|pressured/i },
      { label: 'prednisone/steroid confounder preserved', pattern: /prednisone|steroid/i },
      { label: 'thyroid or cannabis confounder preserved', pattern: /TSH|thyroid|cannabis|UDS/i },
      { label: 'SI/HI denial remains visible', pattern: SI_HI_DENIAL_PATTERN },
    ],
    forbidden: [
      { label: 'medical clearance invented', pattern: /medically cleared|cleared for psych|cleared for psychiatric|medical clearance complete/i },
      { label: 'mania diagnosis invented', pattern: /diagnosed with mania|manic episode is confirmed|meets criteria for mania|bipolar mania/i },
      { label: 'thyroid diagnosis invented', pattern: /diagnosed with thyroid|thyroid disorder confirmed|hyperthyroidism confirmed|hypothyroidism confirmed/i },
      { label: 'pending labs reported as normal', pattern: /free T4 (?:normal|within range)|pregnancy test (?:negative|normal)|labs normal/i },
      { label: 'add-on instruction leaked', pattern: /Do not state medically cleared|Flag prednisone|Keep H&P concise/i },
    ],
  },
  {
    id: 'er-upload-inpatient-eval-alcohol-withdrawal-risk-med-conflict',
    title: 'ER upload and inpatient eval packet preserves withdrawal risk, medication conflict, and collateral without inventing detox orders',
    specialty: 'Psychiatry',
    role: 'Psychiatric NP',
    ehr: 'WellSky',
    noteType: 'Inpatient Psych Initial Adult Evaluation',
    customInstructions: 'Use inpatient psych evaluation sections. Treat ER upload data, patient report, and collateral as separate sources. Do not create detox or medication orders.',
    sourceSections: {
      intakeCollateral: [
        'ER visit PDF/OCR pasted from upload, some lines out of order:',
        '- BAL 212 at 03:40, repeat not visible in upload.',
        '- CIWA? line says "8 then 6" but handwriting unclear.',
        '- Ativan 1 mg given in ER per med admin line; reason field blurry.',
        '- Home med list from old discharge: paroxetine 20 mg, quetiapine 50 mg qhs. Pharmacy fill dates not attached.',
        '- Sister collateral from ER note: patient drinks daily and texted "I cannot do this anymore" last night.',
      ].join('\n'),
      clinicianNotes: [
        'Provider live notes, pasted in messy order:',
        '- pt says "I only drink on weekends" but was trembly this morning.',
        '- denies current SI/HI, denies plan/intent today.',
        '- says Paxil was stopped months ago, not sure about Seroquel.',
        '- MSE: mildly tremulous, anxious, guarded, speech normal rate, TP linear, no hallucinations reported.',
        '- no final detox/medication plan entered in this source.',
      ].join('\n'),
      patientTranscript: [
        'Ambient transcript:',
        'Patient: "I am not trying to kill myself. I was drunk and mad."',
        'Patient: "I shake when I stop drinking sometimes, but I do not drink every day."',
      ].join('\n'),
      objectiveData: [
        'Provider Add-On:',
        '- Keep BAL/CIWA/Ativan source-limited.',
        '- Do not state alcohol withdrawal diagnosis is confirmed.',
        '- Do not say Paxil or Seroquel is active if patient reports stopped/uncertain.',
        '- Do not generate detox orders or medication plan.',
      ].join('\n'),
    },
    required: [
      { label: 'ER BAL and CIWA uncertainty preserved', pattern: /BAL|212|CIWA|handwriting unclear|unclear/i },
      { label: 'Ativan ER administration remains source-limited', pattern: /Ativan|lorazepam|ER.*(?:given|admin)|reason.*(?:blurry|unclear|not visible)/i },
      { label: 'collateral risk text and current denial both preserved', pattern: /sister|collateral|cannot do this anymore|denies? (?:current )?(?:SI\/HI|suicidal|homicidal)|not trying to kill myself/i },
      { label: 'patient alcohol-use conflict preserved', pattern: /weekends|daily|drinks daily|conflict|reports.*weekends/i },
      { label: 'home med uncertainty preserved', pattern: /paroxetine|Paxil|quetiapine|Seroquel|stopped|not sure|uncertain/i },
    ],
    forbidden: [
      { label: 'withdrawal diagnosis confirmed from limited source', pattern: /diagnosed with alcohol withdrawal|alcohol withdrawal is confirmed|meets criteria for alcohol withdrawal/i },
      { label: 'detox or benzo order invented', pattern: /start(?:ed)? CIWA protocol|ordered Ativan|lorazepam taper|detox orders? (?:placed|started)/i },
      { label: 'home meds treated as active despite conflict', pattern: /continue(?:d)? (?:Paxil|paroxetine|Seroquel|quetiapine)|active home medications include paroxetine and quetiapine/i },
      { label: 'false low-risk reassurance', pattern: /\blow[-\s]?risk\b|no safety concerns/i },
      { label: 'provider instruction leaked', pattern: /Keep BAL\/CIWA|Do not generate detox|Provider Add-On/i },
    ],
  },
  {
    id: 'uploaded-previous-provider-note-lithium-creatinine-med-rec-conflict',
    title: 'Uploaded prior-provider note keeps lithium/creatinine medication reconciliation conflict without writing a medication decision',
    specialty: 'Psychiatry',
    role: 'Psychiatric NP',
    ehr: 'Tebra/Kareo',
    noteType: 'Outpatient Psych Follow-Up',
    customInstructions: 'Use concise follow-up sections. Treat uploaded prior-provider note as historical context until confirmed today. It is appropriate to label the medication reconciliation discrepancy when the source supports it.',
    sourceSections: {
      intakeCollateral: [
        'Uploaded previous provider note, OCR from scanned PDF:',
        '- Prior dx: bipolar I? schizoaffective? unclear copied-forward list.',
        '- med list says lithum 600 mg qhs, risperidone 2 mg qhs, trazodone 50 qhs.',
        '- Lab page: creatinine 1.42 on 5/02; lithium lvl line cut off and unreadable.',
        '- Prior note says "stable" but assessment date may be copied from older template.',
      ].join('\n'),
      clinicianNotes: [
        'Today provider typing:',
        '- pt says he stopped lithium 3 weeks ago because tremor and thirst.',
        '- still taking risperidone some nights, misses when sleepy.',
        '- mood more irritable, sleep 4-5 hrs, no clear decreased need for sleep.',
        '- denies SI/HI.',
        '- wants to know if he can restart lithium; no medication decision documented yet.',
      ].join('\n'),
      patientTranscript: [
        'Ambient transcript:',
        'Patient: "I quit the lithium because I was shaking and thirsty."',
        'Patient: "I want to restart something, but I do not know if lithium messed with my kidneys."',
      ].join('\n'),
      objectiveData: [
        'Provider Add-On:',
        '- Do not say lithium restarted or continued.',
        '- Keep creatinine and unreadable lithium level as review items.',
        '- Do not confirm bipolar or schizoaffective from uploaded note labels.',
        '- Mention medication reconciliation discrepancy.',
      ].join('\n'),
    },
    required: [
      { label: 'uploaded prior note remains historical/source-limited', pattern: /uploaded|previous provider|prior note|historical|source-limited/i },
      { label: 'lithium stopped today detail preserved', pattern: /stopped lithium|quit.*lithium|lithium.*3 weeks/i },
      { label: 'tremor/thirst and creatinine preserved', pattern: /tremor|shaking|thirst|creatinine|1\.42/i },
      { label: 'unreadable lithium level preserved', pattern: /lithium (?:lvl|level).*?(?:unreadable|cut off|not visible)|unreadable.*lithium/i },
      { label: 'diagnosis labels remain unconfirmed', pattern: /bipolar|schizoaffective|unclear|not confirmed|prior dx/i },
      { label: 'SI/HI denial remains visible', pattern: SI_HI_DENIAL_PATTERN },
    ],
    forbidden: [
      { label: 'lithium decision invented', pattern: /(?:plan|provider|clinician|prescriber).{0,80}(?:restart(?:ed)?|continue(?:d)?) lithium|lithium (?:was|is) (?:restarted|continued)|(?:restart|continue) lithium (?:today|now|as plan|was documented)/i },
      { label: 'lithium level invented as normal', pattern: /lithium (?:level|lvl).{0,80}(?:therapeutic|normal|within range)/i },
      { label: 'diagnosis confirmed from uploaded labels', pattern: /diagnosed with (?:bipolar|schizoaffective)|(?:bipolar|schizoaffective) disorder is confirmed/i },
      { label: 'adherence overstated', pattern: /taking risperidone as prescribed|good adherence|fully adherent/i },
      { label: 'provider instruction leaked', pattern: /Do not say lithium|Provider Add-On/i },
    ],
  },
  {
    id: 'school-collateral-adolescent-therapy-intake-risk-source-conflict',
    title: 'Adolescent therapy intake keeps school/family collateral, patient denial, and consent limits source-faithful',
    specialty: 'Therapy',
    role: 'Therapist',
    ehr: 'SimplePractice',
    noteType: 'Therapy Intake',
    customInstructions: 'Use therapy intake sections. Keep school collateral, parent report, and youth report distinct. Include safety-plan status if the source says it was started, unfinished, or not uploaded. Do not make legal/consent claims.',
    sourceSections: {
      intakeCollateral: [
        'School referral + parent email pasted together:',
        '- Counselor report: student wrote "I dont want to be here" on assignment last week.',
        '- Parent email: isolating in room, grades dropped, vaping suspected.',
        '- ROI for school not signed yet; referral was brought by parent.',
        '- Past IEP? OCR says "504?" but unclear.',
      ].join('\n'),
      clinicianNotes: [
        'Therapist notes, out of order:',
        '- youth says "I meant school, not life" and denies current SI plan intent.',
        '- mood "annoyed", affect constricted, guarded, tearful when discussing friend conflict.',
        '- reports poor slep, low motivation, vaping "sometimes" but no details.',
        '- safety plan started with parent in waiting room, but final written plan not uploaded yet.',
      ].join('\n'),
      patientTranscript: [
        'Ambient transcript:',
        'Client: "I am not trying to die. I just hate being at that school."',
        'Client: "Please do not tell them everything I said."',
      ].join('\n'),
      objectiveData: [
        'Provider Add-On:',
        '- Do not say no suicide risk.',
        '- Do not state school ROI is signed.',
        '- Do not diagnose MDD or vaping disorder.',
        '- Keep consent/confidentiality as discussion topic only.',
      ].join('\n'),
    },
    required: [
      { label: 'school collateral statement preserved', pattern: /school|counselor|I don'?t want to be here|assignment/i },
      { label: 'patient clarification and current denial preserved', pattern: /meant school|not life|not trying to die|denies?.{0,80}(?:plan|intent)/i },
      { label: 'parent collateral preserved', pattern: /parent|isolating|grades dropped|vaping suspected/i },
      { label: 'ROI/504 uncertainty preserved', pattern: /ROI|not signed|504|unclear|OCR/i },
      { label: 'safety plan status remains incomplete/source-limited', pattern: /safety plan.*(?:started|initiated|in progress|incomplete|not uploaded|not yet uploaded|not finalized)|final written safety plan.*(?:not uploaded|not yet uploaded|not available)/i },
    ],
    forbidden: [
      { label: 'false no-risk reassurance', pattern: /\bno suicide risk\b|\bno risk\b|\blow[-\s]?risk\b|safe from self-harm/i },
      { label: 'ROI invented', pattern: /school ROI (?:signed|completed)|ROI is signed|consent signed/i },
      { label: 'diagnosis invented', pattern: /diagnosed with (?:MDD|major depressive|vaping disorder|nicotine use disorder)|meets criteria for/i },
      { label: 'confidentiality/legal claim invented', pattern: /confidentiality was guaranteed|provider determined legal consent|legally consented/i },
      { label: 'provider instruction leaked', pattern: /Do not say no suicide risk|Do not state school ROI|Provider Add-On/i },
    ],
  },
  {
    id: 'medical-hp-scanned-discharge-summary-qtc-syncope-antipsychotic-confounder',
    title: 'Psych H&P from scanned discharge summary preserves QTc/syncope uncertainty and avoids antipsychotic plan invention',
    specialty: 'Psychiatry',
    role: 'Physician Assistant',
    ehr: 'Oracle Health/Cerner',
    noteType: 'Psychiatric H&P',
    customInstructions: 'Use concise psych H&P format. Preserve medical confounders and pending review items without creating medication changes.',
    sourceSections: {
      intakeCollateral: [
        'Scanned hospital discharge summary OCR, pasted before current HPI:',
        '- syncope eval 2 mo ago; QTc 497? maybe 487? scanned number blurry.',
        '- potassium was 3.3 then, corrected before dc per summary.',
        '- old med list includes ziprasidone 40 bid and ondansetron prn.',
        '- no current EKG in today packet.',
      ].join('\n'),
      clinicianNotes: [
        'Current psych H&P live note:',
        '- patient admitted for paranoia and poor sleep; says "people are tracking me."',
        '- denies chest pain, denies fainting today, denies SI/HI.',
        '- reports stopped Geodon weeks ago because made heart race, but unsure exact date.',
        '- MSE: anxious, paranoid ideation, thought process circumstantial, no clear hallucinations endorsed.',
      ].join('\n'),
      patientTranscript: [
        'Ambient transcript:',
        'Patient: "That Geodon made my heart pound. I am scared of taking it again."',
        'Patient: "I have not passed out today."',
      ].join('\n'),
      objectiveData: [
        'Provider Add-On:',
        '- Do not say current QTc is 497 because scanned value unclear and no current EKG.',
        '- Do not restart or discontinue ziprasidone.',
        '- Preserve syncope/QTc/low potassium history as medical review items.',
      ].join('\n'),
    },
    required: [
      { label: 'scanned QTc uncertainty preserved', pattern: /QTc|497|487|blurry|unclear|scanned/i },
      { label: 'syncope/low potassium history preserved', pattern: /syncope|fainting|potassium|3\.3/i },
      { label: 'no current EKG preserved', pattern: /no current EKG|current EKG.*not|EKG.*not.*today/i },
      { label: 'ziprasidone/Geodon stopped and palpitation concern preserved', pattern: /ziprasidone|Geodon|heart (?:pound|race)|stopped/i },
      { label: 'SI/HI and chest pain/fainting denials preserved', pattern: /denies?.{0,120}(?:SI\/HI|suicidal|homicidal)|denies?.{0,120}(?:chest pain|fainting)/i },
    ],
    forbidden: [
      { label: 'current QTc invented as exact', pattern: /current QTc (?:is|was) 497|QTc is 497 today|QTc 497 today/i },
      { label: 'medication action invented', pattern: /restart(?:ed)? ziprasidone|discontinue(?:d)? ziprasidone|Geodon (?:restarted|discontinued|continued)/i },
      { label: 'cardiac clearance invented', pattern: /cardiac workup normal|cleared by cardiology|medically cleared|EKG normal/i },
      { label: 'psychosis diagnosis overconfirmed', pattern: /diagnosed with schizophrenia|primary psychotic disorder confirmed/i },
      { label: 'provider instruction leaked', pattern: /Do not say current QTc|Preserve syncope|Provider Add-On/i },
    ],
  },
  {
