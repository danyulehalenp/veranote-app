...
export function buildVeraMemoryLedger(input: {
  ...
  pushIfValue(items, address ? {
    ...
    detail: `Atlas should address this provider as ${address}. Preference mode: ${settings.veraAddressPreference}.`,
  ...
  pushIfValue(items, {
    ...
    detail: `Atlas should use a ${settings.veraInteractionStyle} tone with ${settings.veraProactivityLevel} proactivity.`,
    source: 'provider-settings',
    status: 'accepted',
    confidence: 'strong',
    originSummary: 'Saved directly in provider relationship settings.',
  ...
  pushIfValue(items, settings.veraMemoryNotes.trim() ? {
    ...
    status: 'accepted',
    confidence: 'strong',
    originSummary: 'Entered directly by the provider as a durable Atlas memory note.',
    reinforcementSummary: 'Reinforced whenever the provider updates Atlas’s long-term memory note.',
    lastUpdatedAt: nowIso(),
  } : null);
  ...
  Object.entries(learningStore.rewritePreferencesByNoteType || {}).forEach(([noteType, counts]) => {
    (Object.entries(counts) as Array<[string, number]>).forEach(([tone, count]) => {
      ...
      items.push({
        ...
        detail: `Atlas has observed the ${tone.replace(/-/g, ' ')} rewrite style ${count} times for ${noteType}.`,
        source: 'assistant-learning',
        status: 'observed',
  ...
  Object.entries(learningStore.lanePreferencesByNoteType || {}).forEach(([noteType, records]) => {
    records.forEach((record) => {
      ...
      items.push({
        ...
        detail: `Atlas has observed the same lane setup ${record.count} times for ${noteType}.`,
        source: 'assistant-learning',
        status: 'observed',
  ...
  Object.entries(learningStore.promptPreferencesByNoteType || {}).forEach(([noteType, records]) => {
    records.forEach((record) => {
      ...
      items.push({
        ...
        detail: `Atlas has observed the prompt pattern "${record.label}" ${record.count} times for ${noteType}.`,
        source: 'assistant-learning',
        status: 'observed',
        confidence: confidenceForObservedCount(record.count),
  ...
  pushIfValue(items, settings.closerToSourceDefault ? {
    ...
    detail: 'Atlas should preserve source fidelity and default to closer-to-source behavior when uncertainty is present.',
    source: 'provider-settings',
    status: 'accepted',
    confidence: 'strong',
  ...
  pushIfValue(items, settings.outputDestination !== 'Generic' ? {
    ...
    detail: `Atlas should respect ${settings.outputDestination} output constraints without changing meaning or certainty.`,
    source: 'provider-settings',
    status: 'accepted',
    confidence: 'strong',
