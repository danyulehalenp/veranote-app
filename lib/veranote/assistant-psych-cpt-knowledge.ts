...
export function buildPsychCptHelp(normalizedMessage: string, currentDraftText?: string): AssistantResponsePayload | null {
  ...
  if (wantsBillingChecklist(normalized)) {
    return payload(
      'For a quick billing-support checklist, Atlas should answer in a short pre-sign format: likely family, what supports it, what is missing, what red flags remain, and what to verify before signing. She should stay documentation-dependent and not pretend final billing certainty from a short prompt alone.',
      [
  ...
  if (noteSupportText && asksIfNoteSupportsFamily(normalized)
    ...
    if (thinPsychotherapy) {
      return payload(
        'This note sounds thin for a psychotherapy add-on family like 90833, 90836, or 90838. I do not hear clearly supportable psychotherapy work plus psychotherapy timing yet, so Atlas should treat this as a billing-support concern rather than assuming the add-on is justified.',
        [
  ...
  if (noteSupportText && asksIfNoteSupportsFamily(normalized)
    ...
    return payload(
      'This note sounds thin for 90785 based on what is here. I do not hear clearly documented communication-complexity factors yet, so Atlas should treat this as a billing-support concern instead of assuming interactive complexity is supported.',
      [
  ...
  if (noteSupportText && asksIfNoteSupportsFamily(normalized)
    ...
    return payload(
      'This note sounds thin for group psychotherapy as written. I do not hear enough actual group psychotherapy content yet, so Atlas should treat this as a billing-support concern instead of assuming the group family is justified.',
      [
  ...
  if (noteSupportText && asksIfNoteSupportsFamily(normalized)
    ...
    return payload(
      'This note sounds thin for crisis psychotherapy as written. I do not hear clearly documented crisis-intervention work plus crisis timing yet, so Atlas should treat this as a billing-support concern instead of assuming 90839 or 90840 is supported.',
      [
  ...
  if ((hasKeyword(normalized, ['new vs established', 'established vs new', 'new patient', 'established patient'])
    ...
    return payload(
      'For outpatient psych E/M, new versus established status changes the office or outpatient family before you even decide whether psychotherapy add-ons belong. Atlas should treat that as a relationship-to-the-practice question, not as a complexity shortcut or a synonym for intake versus follow-up.',
      [
  ...
  if ((hasKeyword(normalized, ['collateral', 'care coordination', 'family discussion', 'family meeting']) && hasKeyword(normalized, ['family therapy', 'family psychotherapy', '90846', '90847']))
    ...
    return payload(
      'Collateral gathering or family discussion is not automatically family psychotherapy. Atlas should distinguish true family psychotherapy from collateral history, care coordination, or supportive family conversation unless the documented work clearly reflects psychotherapy focused on treatment of the mental disorder.',
      [
  ...
  if ((hasKeyword(normalized, ['interactive complexity']) && hasKeyword(normalized, ['misuse', 'warning', 'warnings', 'red flag', 'red flags', 'not use', 'when not']))
    ...
    return payload(
      'Interactive complexity should be treated cautiously. Atlas should warn that 90785 is not a routine add-on for difficult visits, emotional intensity, or general family involvement; it needs documented communication complications that materially changed the work.',
      [
        ...
        'If the note cannot explain what specifically complicated communication or care delivery, Atlas should stay conservative and avoid implying 90785 is supported.',
      ],
    );
  }
  ...
  if ((hasKeyword(normalized, ['add-on misuse', 'add on misuse', 'misuse warning', 'misuse warnings']) && hasKeyword(normalized, ['90833', '90836', '90838', 'psychotherapy add-on']))
    ...
    return payload(
      'Psychotherapy add-on misuse usually happens when the note really reads like E/M only and then tries to backfill therapy justification. Atlas should warn that add-on families require separately supportable psychotherapy plus the medical visit, not just supportive counseling wrapped around prescribing.',
      [
        ...
        'If the note would still read like straightforward med management after removing a few therapy phrases, Atlas should treat that as a warning against implying an add-on code is justified.',
      ],
    );
  }
  ...
  if (hasDocumentationCue(normalized)
    ...
    return payload(
      'For psychotherapy add-on support, the note needs to read like both services truly happened: a real E/M visit and separately supportable psychotherapy. Atlas should look for clear medical work, clear psychotherapy work, and documentation that does not collapse them into one vague block.',
      [
  ...
  if (hasDocumentationCue(normalized)
    ...
    return payload(
      'Documentation red flags matter a lot for family, group, and crisis psychotherapy families. Atlas should call out when the note sounds too generic to support the family being discussed instead of pretending the billing family is justified.',
      [
  ...
  if ((hasKeyword(normalized, ['intake']) || hasKeyword(normalized, ['90791', '90792']))
    ...
    return payload(
      'Psychiatric intake families and follow-up families should stay separate. Atlas should treat 90791 and 90792 as diagnostic evaluation or intake families, while routine outpatient follow-up usually lives in the E/M family or E/M plus psychotherapy add-on when the documentation supports both.',
      [
  ...
  if ((hasKeyword(normalized, ['psychotherapy-only', 'psychotherapy only', 'therapy-only', 'therapy only', '90834', '90837', '90832']) || hasKeyword(normalized, ['psychotherapy']))
    ...
    return payload(
      'Psychotherapy-only follow-up and med-management follow-up are different billing families. Atlas should keep psychotherapy-only visits in the psychotherapy family, and keep medication-management follow-up in the E/M family unless separately documented psychotherapy supports an add-on code.',
      [
        'Do not let Atlas convert a medication visit into psychotherapy-only billing just because counseling language appears in the note.',
        'Do not let Atlas imply an E/M plus psychotherapy add-on unless both the medical work and the psychotherapy work are distinctly documented.',
      ],
    );
  }
  ...
  if ((hasKeyword(normalized, ['crisis']) || hasKeyword(normalized, ['90839', '90840']))
    ...
    return payload(
      'Crisis psychotherapy and standard psychotherapy should stay clearly separated. Atlas should reserve 90839 and 90840 for true crisis psychotherapy with documented crisis intervention time and should not blur that with ordinary psychotherapy follow-up codes like 90832, 90834, or 90837.',
      [
        'Urgency alone is not enough; the note should support crisis intervention work and crisis timing explicitly.',
        'If the visit was emotionally intense but did not meet crisis psychotherapy standards, Atlas should stay with the ordinary psychotherapy family instead of upgrading it.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['telehealth', 'telemental', 'video visit', 'virtual visit'])) {
    return payload(
      'For psych telehealth, Atlas should treat CPT selection as the same underlying service-family question first, then layer payer-specific telehealth rules, modifiers, and place-of-service requirements on top. She should not pretend telehealth automatically changes the underlying psychiatry family code by itself.',
      [
  ...
  if (hasKeyword(normalized, ['90791']) && hasKeyword(normalized, ['90792']) || hasKeyword(normalized, ['90791 vs 90792', '90792 vs 90791'])) {
    return payload(
      'The key difference between 90791 and 90792 is whether medical services are part of the psychiatric diagnostic evaluation. Atlas should treat 90791 as diagnostic evaluation without medical services and 90792 as diagnostic evaluation with medical services.',
      [
        ...
        'Do not let Atlas collapse a medical-services intake into 90791 or a non-medical psychotherapy intake into 90792 just because the visit was comprehensive.',
      ],
    );
  }
  ...
  if ((hasKeyword(normalized, ['90834']) && hasKeyword(normalized, ['90837'])) || hasKeyword(normalized, ['90834 vs 90837', '90837 vs 90834'])) {
    return payload(
      'The difference between 90834 and 90837 is primarily psychotherapy time, not just visit complexity. Atlas should treat both as psychotherapy-only family codes and keep the distinction tied to the documented therapy duration and actual psychotherapy work.',
      [
        'Do not let Atlas infer 90837 just because the visit felt intense or clinically complicated if the documented psychotherapy time does not support it.',
  ...
  if ((hasKeyword(normalized, ['e/m']) || hasKeyword(normalized, ['evaluation and management']) || hasKeyword(normalized, ['med management', 'med check']))
    ...
    return payload(
      'E/M alone versus E/M plus 90833 comes down to whether separately supportable psychotherapy was provided and documented in addition to the medical visit. Atlas should treat 90833 as an add-on only when the note clearly supports both the E/M work and distinct psychotherapy time/content.',
      [
        'If the visit was primarily medication management, symptom review, and prescribing without separately documented psychotherapy, Atlas should stay with the E/M family alone.',
        'Do not let Atlas backfill therapy language just to justify 90833 after the fact.',
      ],
    );
  }
  ...
  if ((hasKeyword(normalized, ['90846']) || hasKeyword(normalized, ['90847']) || hasKeyword(normalized, ['family therapy', 'family psychotherapy']))
    ...
    return payload(
      'Family therapy and group therapy are different psychotherapy families. Atlas should treat 90846 and 90847 as family psychotherapy codes, 90849 as multiple-family group psychotherapy, and 90853 as group psychotherapy rather than using them interchangeably.',
      [
        ...
        'Do not let Atlas infer family-therapy billing from generic collateral or participation language, and do not let her infer group psychotherapy from attendance alone.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['90792'])) {
    return payload(
      'CPT 90792 is psychiatric diagnostic evaluation with medical services. In practice, Atlas should treat it as the diagnostic-evaluation family that includes medical assessment or prescribing work, not as a generic psychotherapy follow-up code.',
      [
  ...
  if (hasKeyword(normalized, ['90791'])) {
    return payload(
      'CPT 90791 is psychiatric diagnostic evaluation without medical services. Atlas should treat it as an intake or diagnostic-evaluation family code, not as routine medication-management or psychotherapy follow-up billing.',
      [
  ...
  if (hasKeyword(normalized, ['90839', '90840', 'psychotherapy crisis', 'crisis psychotherapy'])) {
    return payload(
      'Psychotherapy for crisis uses the 90839 and 90840 family. Atlas should frame that family as crisis-specific, time-based, and dependent on clearly documented crisis intervention time rather than ordinary supportive therapy language.',
      [
        'Document crisis timing literally and keep the acuity, intervention, and safety context explicit.',
        'Do not let Atlas imply crisis psychotherapy just because the visit was urgent or emotionally intense.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['90785', 'interactive complexity'])) {
    return payload(
      'CPT 90785 is the interactive complexity add-on code used with certain psychiatric services when communication factors substantially complicate the work. Atlas should treat it as an add-on that requires specific communication-complexity support, not as a routine companion code for every therapy visit.',
      [
        'Interactive complexity should stay tied to what actually complicated communication or care delivery in the encounter.',
        'Atlas should stay conservative and avoid implying 90785 unless those complicating factors are clearly documented.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['90833', '90836', '90838', 'psychotherapy add-on', 'add on psychotherapy'])) {
    return payload(
      'Codes 90833, 90836, and 90838 are psychotherapy add-on codes used with an E/M service on the same day when psychotherapy is separately documented. Atlas should treat them as combination-family codes, not as stand-alone med-management shortcuts.',
      [
        'The psychotherapy time and the E/M work both need to be supportable in the note.',
        'If psychotherapy was not separately documented, Atlas should stay conservative and avoid implying an add-on code is justified.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['90832', '90834', '90837']) || (hasKeyword(normalized, ['psychotherapy']) && !hasKeyword(normalized, ['add-on', 'add on', '90833', '90836', '90838', 'crisis']))) {
    return payload(
      'The psychotherapy family 90832, 90834, and 90837 is time-based psychotherapy rather than medication-management E/M. Atlas should treat that family as dependent on actual documented psychotherapy time and therapy content, not just a psychiatric follow-up visit.',
      [
  ...
  if (hasKeyword(normalized, ['90846', 'family psychotherapy without patient', 'family psychotherapy w/o patient'])) {
    return payload(
      'CPT 90846 is family psychotherapy without the patient present. Atlas should treat it as a family-therapy code rather than an individual psychotherapy or med-management code.',
      [
        ...
        'Do not let Atlas blur 90846 with simple collateral gathering or routine care coordination.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['90847', 'family psychotherapy with patient', 'family psychotherapy w patient'])) {
    return payload(
      'CPT 90847 is family psychotherapy with the patient present. Atlas should keep that separate from individual psychotherapy and from pure family-only sessions.',
      [
        ...
        'If medical decision-making or prescribing also occurred, Atlas should stay careful about not collapsing distinct billing families into one undocumented shortcut.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['90849', 'multiple family group'])) {
    return payload(
      'CPT 90849 is multiple-family group psychotherapy. Atlas should treat it as a group-family psychotherapy family rather than ordinary individual or conjoint family therapy.',
      [
        'Documentation should support group-family psychotherapy rather than a standard individual session.',
        'Atlas should not infer 90849 from generic group participation language alone.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['90853', 'group psychotherapy'])) {
    return payload(
      'CPT 90853 is group psychotherapy. Atlas should treat it as a therapy-group family code rather than med-management or individual psychotherapy.',
      [
        'Documentation should support actual group psychotherapy content rather than generic attendance or class participation.',
        'If interactive complexity is being considered, Atlas should keep that as a separate add-on discussion rather than automatically pairing codes.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['90845', 'psychoanalysis'])) {
    return payload(
      'CPT 90845 is psychoanalysis. Atlas should treat it as a distinct psychotherapy family rather than folding it into routine outpatient psychotherapy or med-management follow-up logic.',
      [
        'This is a therapy-specific family and should not be implied from generic supportive therapy wording.',
        'Keep Atlas conservative unless the documentation truly supports that level and type of psychotherapy service.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['99202', '99203', '99204', '99205', '99212', '99213', '99214', '99215', 'e/m', 'evaluation and management', 'med management', 'med check'])) {
    return payload(
      'Psych medication-management follow-up usually lives in the office or outpatient E/M family rather than one single psychiatry-specific CPT code. Atlas should frame that as depending on new versus established status plus the documented medical decision making or total time.',
      [
        ...
        'Atlas should never pretend to choose the final E/M level without the actual documentation support.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['g2211'])) {
    return payload(
      'G2211 is an add-on complexity code tied to certain office or outpatient E/M visits rather than a standalone psych therapy code. Atlas should treat it as documentation-dependent and payer-rule-sensitive, not something to append automatically.',
      [
        'Use the current CMS guidance to confirm when G2211 may be reported with the underlying E/M family.',
        'Keep Atlas conservative unless the continuity and complexity rationale is clearly documented.',
      ],
    );
  }
  ...
  if (hasKeyword(normalized, ['cpt', 'billing', 'modifier'])) {
    return payload(
      'For psych billing help, Atlas should stay inside bounded families rather than pretending final CPT certainty. The main families are psychiatric diagnostic evaluation, office/outpatient E/M, psychotherapy, psychotherapy add-on with E/M, and psychotherapy for crisis.',
      [
        ...
        'Atlas should keep CPT guidance documentation-dependent and avoid implying final billing certainty.',
      ],
    );
  }
